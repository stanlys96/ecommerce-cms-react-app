...
class Cart {
  static async getCart(user_id) {
    try {
      const cart = await pool.query("SELECT P.*, C.quantity, C.cart_id, C.user_id FROM products P INNER JOIN cart C ON C.product_id = P.product_id INNER JOIN users U ON U.user_id = C.user_id WHERE C.user_id = $1 ORDER BY C.cart_id ASC;", [user_id]);
      return cart.rows;
    } catch (err) {
      console.log(err);
    }
  ...
  static async deleteCart(cart) {
    try {
      ...
      return { ...query.rows[0], message: 'Success' };
    } catch (err) {
      console.log(err);
    }
  }
